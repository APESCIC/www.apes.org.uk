...
Constrained the shared mobile menu to vertical scrolling only, so mobile visitors no longer need to pan right to reveal menu items or submenu content.

### Detailed changes

- Added x-axis overflow guards to the opened mobile nav overlay, panel shell and scrollable menu list while preserving vertical scrolling within the menu body.
- Added mobile-specific width guards for mega-menu panels, pill rows, cards and copy blocks so nested submenu content cannot widen the overlay and trigger sideways scrolling.
- Left desktop navigation layout, desktop mega-menu positioning and shared navigation JavaScript unchanged because the regression was resolved in shared mobile CSS only.
- Synchronised the shared release metadata, README, version files and release-facing generated website records to `v2.8.6`, manually updating the exported files because the standard PHP static export script is still unavailable in this shell.
...
- Page or route: shared header navigation, shared CSS, Change Log Hub, root and public release records, README, branded error pages and generated public release records
- Files changed: shared CSS, shared site data release records, VERSION files, root CHANGELOG, public CHANGELOG, README and manually synchronised public release-facing HTML snapshots
...
- Public impact: mobile visitors can scroll the open menu vertically without needing to discover horizontal scrolling to reach hidden items
- Internal impact: the shared mobile overlay now explicitly suppresses x-axis overflow, reducing the risk of future submenu-width regressions
...
- Reason for version bump: shared public-facing mobile navigation overflow hotfix without any route, SEO or metadata restructuring

### Validation

- Checks run: shared mobile-nav CSS review, release-metadata consistency review, footer-link consistency review and generated release-record sync review
- Manual checks completed: mobile nav overflow audit for 390px, 430px and 768px breakpoints, desktop nav regression review for 981px, 1366px and 1920px layouts, footer required-link review, APES Newsroom routing review, sitemap/canonical/robots verification-only review and branded 403/404/500 source-page review
...
- Rollback notes: restore the previous shared CSS, version files and release records, then rerun the standard PHP export or manual snapshot sync if the mobile navigation overflow hotfix needs to be reversed
