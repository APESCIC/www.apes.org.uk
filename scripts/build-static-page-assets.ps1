...
$themeRoot = Join-Path $publicRoot "theme"
$themeEntry = Join-Path $themeRoot "site.css"
$searchIndexDirectory = Join-Path $publicRoot "assets\data"
$searchIndexPath = Join-Path $searchIndexDirectory "search-index.json"

function Resolve-ImportedCss {
    param(
    ...
    foreach ($line in ($content -split "`r?`n")) {
        $trimmed = $line.Trim()
        $match = [regex]::Match($trimmed, '^@import url\("(?<relative>[^"]+)"\);$')

        if ($match.Success) {
            $importPath = Join-Path $directory $match.Groups["relative"].Value
            [void] $builder.AppendLine(("/* Source: {0} */" -f ($importPath.Replace($publicRoot, "").TrimStart("\").Replace("\", "/"))))
            [void] $builder.AppendLine((Resolve-ImportedCss -Path $importPath))
            continue
        }
...
function Get-AssetNamesForPage {
    param(
        [Parameter(Mandatory = $true)]
        [string] $HtmlPath
    )

    $fileName = Split-Path -Leaf $HtmlPath
...
function Convert-HtmlToPlainText {
    ...
    $withoutScripts = [regex]::Replace($Html, '<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>', ' ', 'IgnoreCase, Singleline')
    $withoutStyles = [regex]::Replace($withoutScripts, '<style\b[^<]*(?:(?!<\/style>)<[^<]*)*<\/style>', ' ', 'IgnoreCase, Singleline')
...
function Get-RouteForHtmlFile {
    param(
        [Parameter(Mandatory = $true)]
        [string] $HtmlPath
    )

    $relative = $HtmlPath.Substring($publicRoot.Length).TrimStart("\")

    if ($relative -ieq "index.html") {
        return "/"
    }

    if ($relative -match '^(403|404|500)\.html$') {
        return "/$relative"
    }

    $withoutIndex = $relative -replace '\\index\.html$', ''

    return "/" + ($withoutIndex.Replace("\", "/").Trim("/") + "/")
}
...
function initStaticSearch() {
  const resultsRegion = document.querySelector("[data-static-search-results]");
  const searchInput = document.querySelector("#site-search");
  const searchStatus = document.querySelector("[data-search-status]");
  const resultsList = document.querySelector("[data-search-result-list]");

  if (!resultsRegion || !searchInput || !searchStatus || !resultsList) {
    return;
  }
  ...
  if (query === "") {
    renderMessage("Enter a search term to see results.");
    return;
  }
  ...
  fetch("/assets/data/search-index.json", { headers: { Accept: "application/json" } })
    ...
    .then((entries) => {
      ...
      if (!results.length) {
        renderMessage(`No results found for "${query}". Try a route, service, policy or support topic.`);
        return;
      }
        ...
        .map((entry) => `
          <li class="search-result-item">
            <article class="mini-panel">
              ...
              <h3><a class="text-link" href="${entry.route}">${escapeHtml(entry.title)}</a></h3>
              <p>${escapeHtml(entry.description)}</p>
              <p><a class="text-link" href="${entry.route}">${escapeHtml(entry.route)}</a></p>
            </article>
          </li>
        `)
        .join("");
    ...
    .catch(() => {
      renderMessage("Search results are temporarily unavailable. Please try again or use the main navigation.");
    });
}
...
$canGeneratePageAssets = Test-Path -LiteralPath $themeEntry -PathType Leaf
$flattenedCss = if ($canGeneratePageAssets) {
@"
/* Page-owned stylesheet generated from the former shared APES theme. */
/* Each page now ships with its own local stylesheet for independent editing. */

$(Resolve-ImportedCss -Path $themeEntry)
"@
} else {
    ""
}
...
$htmlFiles = @(
    Get-ChildItem -LiteralPath $publicRoot -Recurse -File -Filter "index.html"
    Get-ChildItem -LiteralPath $publicRoot -File | Where-Object { $_.Name -in @("403.html", "404.html", "500.html") }
)

if ($canGeneratePageAssets) {
    foreach ($file in $htmlFiles) {
        $assetNames = Get-AssetNamesForPage -HtmlPath $file.FullName
        $cssPath = Join-Path $file.DirectoryName $assetNames.Css
        $jsPath = Join-Path $file.DirectoryName $assetNames.Js
        $html = Get-Content -LiteralPath $file.FullName -Raw

        $html = [regex]::Replace(
            $html,
            '<link rel="stylesheet" href="[^"]*theme/site\.css" />',
            ('<link rel="stylesheet" href="{0}" />' -f $assetNames.Css)
        )

        $html = [regex]::Replace(
            $html,
            '<script type="module" src="[^"]*theme/js/site\.js" defer></script>',
            ('<script type="module" src="{0}" defer></script>' -f $assetNames.Js)
        )

        if ($file.FullName -ieq (Join-Path $publicRoot "search\index.html")) {
            $html = [regex]::Replace(
                $html,
                '(?s)<section class="section-shell">\s*<div class="section-heading">\s*<p class="eyebrow">Results</p>\s*<h2>Search the site to see results\.</h2>\s*</div>\s*</section>',
                @'
<section class="section-shell" data-static-search-results>
  <div class="section-heading">
    <p class="eyebrow">Results</p>
    <h2>Search results</h2>
  </div>
  <p data-search-status>Enter a search term to see results.</p>
  <ul class="clean-list" data-search-result-list></ul>
</section>
'@
            )
        }

        Set-Content -LiteralPath $cssPath -Value $flattenedCss
        Set-Content -LiteralPath $jsPath -Value $pageRuntime
        Set-Content -LiteralPath $file.FullName -Value $html
    }
}

if (-not (Test-Path -LiteralPath $searchIndexDirectory -PathType Container)) {
    New-Item -ItemType Directory -Path $searchIndexDirectory -Force | Out-Null
}

$searchEntries = foreach ($file in (Get-ChildItem -LiteralPath $publicRoot -Recurse -File -Filter "index.html")) {
    $html = Get-Content -LiteralPath $file.FullName -Raw
    $titleMatch = [regex]::Match($html, '<title>(?<value>.*?)</title>', 'IgnoreCase, Singleline')
    $descriptionMatch = [regex]::Match($html, '<meta name="description" content="(?<value>[^"]*)"', 'IgnoreCase')
    $text = Convert-HtmlToPlainText -Html $html

    [pscustomobject]@{
        title = [System.Net.WebUtility]::HtmlDecode(($titleMatch.Groups["value"].Value -replace '\s+', ' ').Trim())
        description = [System.Net.WebUtility]::HtmlDecode($descriptionMatch.Groups["value"].Value.Trim())
        route = Get-RouteForHtmlFile -HtmlPath $file.FullName
        text = $text
    }
}
...
$searchEntries |
    ...
    Set-Content -LiteralPath $searchIndexPath

if ($canGeneratePageAssets) {
    Write-Host "Generated page-owned CSS/JS assets and static search index for $($htmlFiles.Count) public pages."
} else {
    Write-Host "Refreshed the static search index from the current page-owned public pages."
}
